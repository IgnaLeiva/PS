...
- Randomisation (if it works) makes groups being compared balance on baseline characteristics. (observed and unobserved)
...
- An alternative when randomisation is not always feasible
- Selection bias is a major concern (imbalanced groups)
...
RR_data  <- data.frame(Condition = c(rep("Virally non-suppressed: STR vs MTR", 3), rep("Adherent: STR vs MTR",3)),
