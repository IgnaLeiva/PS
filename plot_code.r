...
ggsave(plot = plot_A, "plot/plot_A.tiff", width = 4, height = 4)
...
plot_B <- currentDataset %>% 
  ...
  ggplot(aes(x = ps, group = gender, fill = gender)) +
...
ggsave(plot = plot_B, "plot_B.tiff", width = 4, height = 4)
...
currentDataset %>%
  mutate(gender = ifelse(sex == 0, "Female", "Male")) %>%
ggplot(aes(x = gender, y = ps, fill = gender)) +
    geom_boxplot() +
    scale_fill_viridis(discrete = TRUE, alpha=0.6, option="D") +
    theme_ipsum() +
  coord_flip() +
    ...
    ggsave(plot = plot_c, "plot/plot_C.tiff", width = 4, height = 4)
